...
class tsAnonimo{
	...
	public static function &getInstance(){
		...
		global $babanta, $wuser, $cookievalor;
	...
	*/
	function logoutUser($user_id, $redirectTo = '/'){
	  ...
	  if($username && $password){
        $username = $username;
        $infouserbanned = mysql_fetch_assoc(mysql_query("SELECT * FROM babanta_usuarios WHERE buser_nick='".$username."'"));
		if(mysql_num_rows(mysql_query("SELECT buser_nick, buser_pass FROM babanta_usuarios WHERE buser_nick='".$username."' AND buser_pass='".md5($password)."'"))){
		$pp_password = md5($password);
	    }elseif(mysql_num_rows(mysql_query("SELECT buser_nick, buser_pass FROM babanta_usuarios WHERE buser_nick='".$username."' AND buser_pass='".$babanta->encryptIt($password)."'"))){
		$pp_password = $babanta->encryptIt($password);
	    }else{ 
	    $pp_password = $password; 
	    }
		$query = mysql_query("SELECT * FROM babanta_usuarios WHERE buser_nick='".$username."' AND buser_pass='".$pp_password."'");
        if(!$query) $query = mysql_query("SELECT * FROM babanta_usuarios WHERE buser_nick='".$username."' AND buser_pass='".$pp_password."'");
		$data = mysql_fetch_assoc($query);
        $sdfleuaesk = mysql_num_rows(mysql_query("SELECT * FROM babanta_usuarios WHERE buser_nick='".$username."'"));
        $SD65F4S6513 = mysql_fetch_assoc(mysql_query("SELECT * FROM babanta_usuarios WHERE buser_nick='".$username."'"));
		if($sdfleuaesk >= 1){ 
		...
		return '0: Ingresa tus datos, por Dios!!';
		}
	  }
	   /*VALIDA EMAIL */
    ...
    /*$scode = $babanta->setSecure($_POST['recaptcha_response_field']);
	$scode2 =$babanta->setSecure($_POST['recaptcha_challenge_field']);
   ...
   $nonames2 = array('recaptcha', 'configurar', 'cuenta', 'settings', 'configuracion', 'configuración', 'configurar', 'arreglar', 'editar','imagenes', 'img', 'images', 'imagen', 'calendario', 'chats', 'chat', 'grupos', 'mail', 'groups', 'foro', 'fotos', 'foto', 'photos', 'galeria', 'galery');
    ...
    if($buser_nick == NULL or $sjdokf == NULL or $_POST['reclave'] == NULL or $buser_mail == NULL){
    ...
    }else{ $passwordvalid = $babanta->checkPass($_POST['clave']); if($passwordvalid){
...
if(mysql_query("INSERT INTO babanta_usuarios(buser_nick, buser_pass, buser_mail, buser_register, buser_network, babanta_ref) VALUES('".$buser_nick."', '".$buser_pass."', '".$buser_mail."', '".$identi."', '".$babanta->settings['net_id']."', '".$babantareforg."')")){
...
// DAR BIENVENIDA
//$tsWall->Darbienvenida_newuser($insertId);
//// ENVIAR CORREO ////
$para = $buser_mail;
$titulo = 'Información de tu cuenta en'.$babanta->settings['net_name'];
...
$asunto = $titulo;
$body = array(
'top' => 'Te damos la bienvenida a '.$babanta->settings['net_name'].', queremos invitarte a compartir y conocer personas en Wortit diariamente, invitar a tus amigos y seguir formando parte de nosotros.',
'center' => 'Nombre de usuario: '.$buser_nick.'<br>Contraseña: <b>'.$sjdokfSS.'</b>****',
'bottom' => '',
'link' => array('link' => $babanta->settings['url'].'?getto=welcome', 'name' => 'Ir&nbsp;a&nbsp;Wortit'),
'link2' => array('link' => '', 'name' => ''),
);
$altbody = 'Bienvenido a '.$babanta->settings['net_name'].', '.$buser_nick.' ingresa con los siguientes datos: Nmbre de usuario: '.$buser_nick.', Contraseña: '.$sjdokfSS.'****';

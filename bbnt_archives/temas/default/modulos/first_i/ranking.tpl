...
<div class="col-sm-6 col-md-4 item-new">
    <div class="thumbnail">
      <div class="alert alert-info" style="margin: 0;padding: 6px 7px;text-align: center;">{$p.postix_cat_name}</div>
      ...
      <div class="caption">
        <h3>{$p.postix_name}</h3>
        <div class="fb-share-button" data-href="http:{$tsConfig.url}/int/noticias-share/{$p.postix_id}/{$wuser->uid}/" data-layout="button_count"></div>
        <a onclick="window.open('https://twitter.com/home?status=http:{$tsConfig.url}/int/noticias-share/{$p.postix_id}/{$wuser->uid}/', 400, 400);" class="btn btn-warning btn-sm color_white" role="button" style="margin: 9px 0 11px 0;">Compartir en Twitter</a>
        {if $wuser->admod}<a href="{$tsConfig.partner_url}?page=admod&pref={$p.postix_id}&action=editar" class="btn btn-warning btn-sm color_white" role="button" style="margin-bottom: 11px;">Editar nota</a>{/if}
...
<script type="text/javascript" src="{$tsConfig.url}/js/code/masonry.pkgd.min.js"></script>

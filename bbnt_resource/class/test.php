...
$infocobro = mysql_query("SELECT c.c_user_id,c.c_dinero,u.buser_id,u.buser_register FROM u_cobros AS c LEFT JOIN babanta_usuarios AS u ON c.c_user_id = u.buser_id WHERE c.c_type='1'");
...
while ($row = mysql_fetch_assoc($infocobro)){
	...
	if($isbanned <= 0) { 
		$valores_plata[$row['c_user_id'].'%'.$row['buser_register']]=$new_cash = floatval(str_replace(',', '.', $row['c_dinero']));
...
<hr>
<center><form method="GET" action=""><table width="800px" border="1" style="margin:auto;">
<tbody>
<tr>
<td>Filtrar por:</td>
<td><label>Dia&nbsp;</label><select name="day">
<?php
for($i=1;$i<31;$i++){
...
<td><label>Mes&nbsp;</label><select name="month">
<?php
for($i=1;$i<12;$i++){
echo '<option value="'.$i.'">'.$i.'</option>';
}
...
<?
	...
	foreach ($valores_plata as $useridinfo =>$plata){
		...
		echo $data[0].' => '.$plata."; ".$this_day.' de '.$this_moth."<br>";
	    }
	}
}
?>